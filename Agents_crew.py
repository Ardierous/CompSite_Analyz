...
if not CREWAI_IMPORTED:
    ...
    memory=False,  # иначе возможна подмешка контекста между разными анализами в одном процессе
    )
        # #region agent log
        try:
            with open(debug_log_path, 'a', encoding='utf-8') as f:
    ...
    llm_config={
        "model": "gpt-4o",
        "temperature": 0.2,  # ниже — меньше домыслов вне текста Задачи 1
    ...
    verbose=True,
    allow_delegation=False,  # без делегирования — меньше ухода от фактов Задач 1–2
    tools=[],  # Нет инструментов - работает только с предоставленными данными
    
    llm_config={
        "model": "gpt-4o",
        "temperature": 0.25,  # строже к фактам из Задач 1–2
    ...
    ЗАПРЕЩЕНО дополнять отчёт «типичными для отрасли» фактами, цифрами и названиями, которых НЕТ в тексте Задачи 1.
    Если данных нет — пишите явно: «на сайте не указано», а не обобщайте из внешних знаний.
    Ссылки [текст](url) — ТОЛЬКО если такой URL буквально встречается в Задаче 1; иначе без ссылки.
    ...
    Ссылки на источники — только URL из Задачи 1, формат [текст](url).""",
    ...
    КРИТИЧЕСКИ ВАЖНО:
    1. ДЕТАЛИЗАЦИЯ: Каждый пункт — минимум 3–5 предложений. Только факты из Задач 1 и 2. Не опускайте информацию, но и НЕ ПРИДУМЫВАЙТЕ.
    2. НЕ ЗАПОЛНЯЙТЕ шаблон выдумкой: если в Задачах 1–2 нет финансов, «инвестиционных проектов», SWOT по сути сайта — пишите «в открытых материалах сайта не представлено», а не общие фразы из интернета.
    3. ССЫЛКИ: В «Источники» и приложении — ТОЛЬКО URL, которые есть в ответах инструментов скрапинга в Задаче 1. НЕ сокращайте и НЕ угадывайте пути (/projects, /about и т.д.), если их нет в тексте Задачи 1.
    4. СТРУКТУРА САЙТА: только пункты с URL из Задачи 1 (список ссылок инструмента). Иначе: «Структура: см. Задачу 1» без вымышленных ссылок.""",
