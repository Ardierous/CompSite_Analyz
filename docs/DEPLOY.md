...
Используйте скрипт для автоматической настройки файрвола и запуска:
...
- Проверяет и настраивает файрвол UFW (открывает порт 5000)
...
Ubuntu 20.04 использует UFW (Uncomplicated Firewall) по умолчанию. Для доступа к приложению извне необходимо открыть порт 5000.
...
2. `scripts/setup_firewall.sh` - скрипт настройки файрвола (опционально)
3. `scripts/start_app.sh` - скрипт автоматического запуска (опционально)
4. `.env` - переменные окружения (создать из `env.example`)
